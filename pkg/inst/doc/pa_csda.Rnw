...
attribute active return to different factors. 
...
% Is this font size shrinkage consistent with The R Journal standards?
...
of the original data set (\texttt{year}, \texttt{quarter}, and \texttt{jan}), each containing 3000 securities. 
The data frame,
...
\item barrid: Barra security identifier.
...
\item momentum: a measure of sustained relative price performance.

\item value: the extent to which a stock is priced
  inexpensively in the market.

\item size: normalized market capitalization.

\item growth: capture stock's growth prospects.

\item cap.usd: market capitalization in USD.

\item yield: dividend yield of a security.

\item country: the country in which the company is traded.

\item currency: currency in which the security is priced.
...
\item portfolio: top 200 securities based on \texttt{value} scores
  ...
  December 2010. They are equal-weighted each month.
...
%% Go through all these with someone to make sure they are correct!
...
country or sector. Unfortuinately, it is very hard to expand the analysis beyond 
two categories.\footnote{\citet{garytwo} proposed a framework
...
effects:
\begin{eqnarray*}
  R_{selection} & = & R_{country \, selection} + R_{sector \, selection} + R_{country \, selection} * R_{sector \, selection}\\ 
