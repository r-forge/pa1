...
examples based on data from MSCI Barra's Global Equity
Model II (GEM2).\footnote{See www.msci.com and \cite{gem2} for more
  information.} The original data set contains selected attributes such as industry,
...
geometric compounding.  Geometric is able to
circumvent the adding-up problem. 
