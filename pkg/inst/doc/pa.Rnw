...
the Brinson Model and the Regression-based analysis. In addition to
demonstrating how to accomplish these two analyses, this article will
show the calculation of the Cross-sectional Volatility (CSV) in the
\pkg{pa} package.


\section*{Background}

To demonstrate the capabilities of the \pkg{pa} package we will
...
provider of investment decision support tools to investment
...
available as part of the \pkg{pa} package. 
...
\texttt{GEM2} contains selected attributes such as sector, size,
country, and various style factors for a universe of 36,000 securities
as benchmark. The data is on a monthly frequency from January, 2010
...
randomly selected as portfolio holdings in January and are held
...
Management in 1986 and has become the best-known approach to
...
<<echo = FALSE>>=
data(JanGEM2)
br.single <- brinson(x = JanGEM2, date.var = "date", sector.var = "sector", 
...
\begin{verbatim}
> data(JanGEM2)
> br.single <- brinson(x = JanGEM2, 
+    date.var = "date", sector.var = "sector", 
...
<<echo = FALSE>>=
br.multi <- brinson(GEM2, date.var = "date", 
                    sector.var = "sector", 
                    bench.weight = "benchmark", 
                    portfolio.weight = "portfolio", 
                    ret.var = "return")
...
<<echo = FALSE>>=
rb.single <- regress(JanGEM2, date.var = "date", 
                     ret.var = "return",
                     reg.var = c("sector", "value", 
                       "size", "growth"),
                     benchmark.weight = "benchmark", 
...
\begin{verbatim}
> rb.multi <- regress(GEM2, date.var = "date", 
+               ret.var = "return",
+               reg.var = c("sector", 
+               "value", "size", "growth"),
+               benchmark.weight = "benchmark", 
...
<<>>=
csv.single <- csvol(JanGEM2, date.var = "date", 
                    ret.var = "return", 
                    uni.var = "benchmark") 
...
\texttt{JanGEM2}. \texttt{date.var} indicates the name of the column
...
<<>>=
csv.multi <- csvol(GEM2, date.var = "date", 
                   ret.var = "return", 
                   uni.var = "benchmark") 
@ 
...
\address{David Kane and Yang Lu \\
  \email{DKane@iq.harvard.edu} and \email{Yang.Lu@williams.edu}}
