...
allocation, stock selection, and interaction effects. The
...
The arithmetic active return measure of the portfolio $R_{active}$ is
...
model is to expand the analysis beyond a single categorical
...
calculate allocation, selection and interaction effect within each
period and link them across time. Two methods of linking attribution
effects over multiple periods exist -- arithmetic and
geometric. Arithmetic measure calculates relative performance of a
portfolio and its benchmark by a difference; geometric measure does so
by a ratio. Arithmetic measure is more intuitive but a well-known
challenge in arithmetic attribution is that active returns do not add
up over multiple periods due to geometric compounding.\footnote{See
  \cite{practical} for a complete discussion of the complexity
  involved.} Geometric measure which appears to be less intuitive, is
able to circumvent the adding-up problem. \cite{multi} discussed
various linking algorithms to connect arithmetic measure with
geometric measure and argued that the \emph{optimized linking
  algorithm} is the best way to link attribution over time.

We present arithmetic, geometric and optimized linking algorithm
below. The \pkg{pa} package offers the same three options for
multi-period attribution which are discussed later in the paper.

\texttt{Arithmetic Attribution.} The arithmetic attribution is to
calculate active return and contributions due to allocation,
selection, and interaction in each period and sum them over multiple
...
and $R_t^P$ and $R_t^B$ are portfolio and benchmark return in a single
...
\emph{natural scaling} $A$ and an adjustment $a_t$,
...
return in each period adjusted for linking, the following is true:
...
represent allocation, selection and interaction effect in each period
$t$, respectively.
...
consider it in the contextof a regression model. A regression can be
run without intercept. Estimated coefficients will be the mean return
of each category in the universe. The mean return of each category
also appears in the Brinson analysis. The equivalent to the allocation
effect for the universe in the Brinson model is the sum of the product
of estimated coefficient of each category and the active weight of
each category, defined by the difference between portfolio weight and
benchmark weight of the category.

In order to estimate the selection effect of each category in the
Brinson analysis, one can calculate the product of the residual of the
aforementioned regression and active weight of each category.

The interaction effect is the difference between the portfolio's
actual return and the sum of the allocation and selection effect.
...
following regression formulae without intercept based on a portfolio
of $n$ securities with $k$ different categories (such as country,
sector, and size):
...
\begin{itemize}
\item $r_n$ is a $1$ by $n$ vector of stock returns from the portfolio.
\item $f_n$, called \emph{factor return}, is a $1$ by $n$ vector of
  estimated coefficients from the regression.
\item $x_{n,k}$ is an $n$ by $k$ vector of stock characteristics.
\item $u_n$ is a $1$ by $n$ vector of residuals.
\end{itemize}

For each of the $k$ categories, the active exposure $X_i$, $i$ $\in$
$k$, is expressed as

\begin{center}
  $X_i = w_{active}'x_{n,i}$.
\end{center}

The contribution of a category $i$, $R_i$, is thus the vector product
of active exposure of the category and the factor return. That is,
...
of contributions of all $k$ categories and the residual,
