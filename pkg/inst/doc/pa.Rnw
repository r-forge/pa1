...
complexity. However, as the number of sub-categories increases, such
procedure does not handle multiple variables well and faces the
...
between a portfolio's sector allocation and the benchmarks's sector
...
$R_{selection}$ is the sum of the country selection effect and the
...
grows exponentially. In the case of $Q$ categories where $Q > 1$,
there are $3^Q - 2Q - 1$ terms for the interaction effect between any
of the two categories. Each of the $Q$ categories has three different
characteristics -- its allocation effect, selection effect and no
effect. Imagine $Q$ categories break the portfolio into $3^Q$
cells. If a cell consists of the allocation effect of a category $i$,
no effect and no effect, it represents the allocation effect of
category $i$. Should the cell include the allocation effect of a
category $j$, the selection effect of another category $k$, and no
effect, the cell represents the interaction between allocation for
category $j$ and selection for category $k$. The number of interaction
terms for $Q$ categories is $3^Q - 2Q - 1$.
...
provided in the \pkg{pa} package to analyze a single period portfolio
