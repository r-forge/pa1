...
to consider it in the context of a regression model. Conducting the
Brinson analysis is similar to running a linear regression without the
intercept term. Estimated coefficients will then be the mean return of
each category in the universe, a.k.a. the factor return of each
...
of all three holdings. The matrix $\mathbf{X_{3,3}}$ records the
scores for each of the three factors (value, growth, and momentum) in
each row. $\mathbf{f_3}$ contains the estimated coefficients when regressing
$\mathbf{r_3}$ on $\mathbf{X_{3, 3}}$. 
...
length $n$ containing the active weight of every security in the
portfolio, and $\mathbf{x_{n}}$ is a column vector of length $n$
...
attribute value, $X_{value}$ is the product of $\mathbf{w_{active}}'$
(containing the active weight of each of the three holdings) and
$\mathbf{x_{3}}$ (containing value scores of the three holdings).
