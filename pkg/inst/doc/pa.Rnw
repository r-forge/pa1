...
multi-period attribution which is discussed later in the paper.

\texttt{Arithmetic Attribution.} The arithmetic attribution is to
calculate active return and contributions due to allocation,
selection, and interaction for each period and sum them over multiple
...
\begin{center}
  $1 + R_P = \prod_{t = 1}^T (1 + R_t^P)$,
  
  $1 + R_B = \prod_{t = 1}^T (1 + R_t^B)$,
\end{center}

and $R_t^P$ and $R_t^B$ are portfolio and benchmark return in a period
$t$, respectively. 
...
\emph{natural scaling} $A$ and an adjustment $a_t$\footnote{See
  \cite{linking} for more information on the optimized linking
  coefficients.}.
