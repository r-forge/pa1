...
multi-period attribution as shown later in the paper.
...
linear regression without intercept based on a portfolio of $n$
securities with $k$ different variables (such as country, sector, and
size):
...
\begin{itemize}
\item $r_n$ is a column vector of length $n$. Each element in $r_n$
  represents the return of a security in the portfolio.
\item $x_{n,k}$ is an $n$ by $k$ vector. Each row represents $k$
  characteristics (variables) of a security. There are $n$ securities
  in the portfolio.
\item $f_n$ is a column vector of length $n$. The elements are
  estimated coefficients from the regression. Each element represents
  the \emph{factor return} of a variable.
\item $u_n$ is a column vector of length $n$. The elements are the
  residuals from the regression.
\end{itemize}

For the active exposure of each of the $k$ variables, $X_i$, $i$ $\in$
$k$, is expressed as
...
where $X_i$ is a column vector of length $k$ representing the active
exposure of a variable for all securities, and $w_{active}$ is a
column vector of length $n$ including the active weight of every
security in the portfolio.

The contribution of a variable $i$, $R_i$, is thus the vector product
of active exposure of the variable and the factor returns. That is,
...
where $u$ is the residual, a.k.a. the interaction effect.
...
\texttt{brinson} reports essential information about the input
...
user-defined category. Here, it shows the exposure on \emph{size}. We
can see that the portfolio overweights the benchmark in size of the
smallest quintile and underweights in that of the highest quintile.
