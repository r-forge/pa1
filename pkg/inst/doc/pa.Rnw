...
having the same sotck selection and thus benchmark return for each
category.  The difference between the two returns is that caused by
the allocation scheme and is called the allocation effect. Similarly,
two different returns can be produced when two portfolios have the
same allocation yet dissimilar returns due to differences in stock
