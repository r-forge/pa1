...
Many portfolio managers measure portfolio performance with reference
to a benchmark. The difference in return between a portfolio and its
benchmark is the active return of the portfolio. Portfolio managers and 
their clients often want to know what caused this active return. Performance
attribution is a technique used to decompose the active return. The
two most common approaches are the Brinson model and a
regression-based analysis.\footnote{See \cite{brinson:gary} and
  \cite{jpmreg} for more information.}

Portfolio managers of various asset classes use different variations
of the two models to assess the performance of their
portfolios. Managers of fixed income portfolios include yield-curve
movements [\cite{lord}] in the model. Managers of multi-currency
assets use a sector-based approach, which is conceptually the same as
the Brinson model, to take into account the currency effect and
incorporate ``local risk premium'' into the attribution model
[\cite{multi}]. This paper focuses on attribution models for equity
portfolios without considering the currency effect.

The \pkg{pa} package provides tools for conducting both methods of
performance attribution for equity portfolios. The Brinson model takes
an ANOVA-type approach and decomposes the active return of any given
portfolio into asset allocation, stock selection, and interaction
effects. The regression-based analysis utilizes estimated coefficients
from a linear model to attribute active return to different
factors. After describing the Brinson and regression approaches and
demostrating their use via the \pkg{pa} package, we show that the
Brinson model is just a special case of the regression approach.
...
The full version of the original GEM2 data for 2010 is in the
...
48,000 securities on a monthly basis.
...
Since the category allocation of the portfolio is generally different from that of the
benchmark, allocation plays a role in the active return
$R_{active}$. The same applies to stock selection where assuming that the
...
produce different returns with different allocation schemes while
having the same sotck selection and thus benchmark return for each category.  The difference
between the two returns is that caused by the allocation scheme and is called the allocation effect. Similarly, two
...
framework to include two variables in the Brinson analysis with added
complexity. However, such procedure does not handle multiple variables
well and faces the \emph{curse of dimensionality} as the number of
sub-categories increases.
...
between a portfolio's sector allocation and the benchmakrs's sector
alocation both with benchmark returns:
\begin{center}
...
\begin{eqnarray*}
  ...
  & = & \sum\limits_{j = 1}^N WC^B_jRC^P_j - \sum\limits_{j = 1}^N WC^B_jRC^B_j \\ 
  & + & \sum\limits_{j = 1}^N WS^B_jRS^P_j - \sum\limits_{j = 1}^N WS^B_jRS^B_j,
\end{eqnarray*}
\end{center}
...
allocation, as well as between country selection and sector selection.
...
selection effect and no effect. Imagine $Q$ categories break the
portfolio into $3^Q$ cells. If a cell consists of the allocation
effect of a category $i$, no effect and no effect, it represents the
allocation effect of category $i$. Should the cell include the
allocation effect of a category $j$, the selection effect of another
category $k$, and no effect, the cell represents the interaction
between allocation for category $j$ and selection for category
$k$. Therefore, the number of interaction terms for $Q$ categories is
$3^Q - 2Q - 1$.
...
\begin{center}
  ...
  \begin{eqnarray*}
    ...
    & + & \sum \limits_{i = 1}^Q \sum \limits_{j = 1}^Q \sum \limits_{k = 1}^Q R_{allocation_i}R_{allocation_j}R_{selection_k} \\
    & + & \dots.
  \end{eqnarray*}
\end{center}
...
produce an object of class \texttt{brinson} (a single-period
portfolio). Below we show the tools provided in the \pkg{pa} package
to analyze a single period portfolio based on the Brinson model.
...
\texttt{jan}, contains all the information necessary to conduct a
single-period Brinson analysis. \texttt{date.var}, \texttt{cat.var},
and \texttt{return} identify the columns containing the date, the
factor to be analyzed, and the return variable,
...
worst performing sector, lost more than 10\%.
...
arithmetic, geometric, and optimized linking. Arithmetic measure
...
geometric compounding.\footnote{See \cite{practical} for a complete
  discussion of the complexity involved.} Geometric is able to
circumvent the adding-up problem. \cite{multi} discussed various
linking algorithms to connect arithmetic return with geometric return
and argued that the \emph{optimized linking algorithm} is the best way
to link attribution over time.
...
of a multi-period Brinson analysis. 
...
\begin{center}
  ...
  %%  $\mathbf{r_n} = \mathbf{a} + \mathbf{X_{n,k}f_k} + \mathbf{u_n}$
\end{center}
where 
\begin{itemize}
...
\item $\mathbf{u}_n$ is a column vector of length $n$. The elements
...
estimated by regressing returns on the attribute specified by the
portfolio manager:
\begin{center}
...
methods in \R{} with the \pkg{pa} package. We also show that the
Brinson model is a special case of the regression method. A
comprehensive package, \pkg{portfolio} [\cite{kane:david}], provides
facilities to calculate exposures and returns for equity
portfolios. It is possible to use the \pkg{pa} package on the output
generated by the \pkg{portfolio} package. Further, the flexibility of
