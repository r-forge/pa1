...
\item $\mathbf{a}$ is a column vector of length $n$ with all the
  intercept terms.
...
active weight of each category. Active weight of a category is defined
as the difference between portfolio weight and benchmark weight for
that category.
