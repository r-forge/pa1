...
applied on real-world data sets by using the tools provided in the
\pkg{pa} package.


\section*{Background}

two paragraphs about brinson, regression

for brinson -- explain the concept of allocation -- give an example,
the weight and allocation of of sectors, SP500 benchmark, but a heavy
weight sector like utilities; explain the formula

for regression -- explain the formula
...
<<echo = FALSE>>=
data(Jan)
br.single <- brinson(x = Jan, date.var = "date", sector.var = "sector", 
