...
methods: the Brinson model and a regression-based
analysis\footnote{See \cite{brinson:gary} and \cite{jpmreg} for more
  information.}.  The Brinson model takes an ANOVA-type approach and
decomposes the active return of any given portfolio into asset
allocation, stock selection, and interaction effect. The
...
factors. This article will show how to apply these methods by using
the \pkg{pa} package.

Many portfolio managers of various asset classes use different
attribution models to assess the performance of their
portfolios. Equity portfolio managers primarily use the Brinson and
regression approaches to decompose returns. Portfolio managers use a
sector-based approach, which is similar to the concept of the Brinson
model, to incorporate ''local risk premium'' into the attribution
model [\cite{multi}]. Further, managers of fixed income portfolios
utilize models that take into account yield-curve movements
[\cite{lord}]. The focus of this paper is on attribution models for
equity portfolios. After describing the Brinson and regression
approaches and demostrating their use via the \pkg{pa} package, we
show that the Brinson model is just a special case of the regression
approach.
...
real-world data sets by MSCI Barra\footnote{See www.msci.com for
  details.}. MSCI Barra is a leading provider of investment decision
...
arithmetic measure for a single-period Brinson model. 
...
Since the allocation of the portfolio is generally different from that of the
...
produce two different returns with different allocation schemes while
having the same benchmark return for each category.  The difference
between the two returns is thus the allocation effect. Similarly, two
different returns can be produced when two portfolios have the same
allocation yet dissimilar returns due to differences in stock
selection within each category. This difference is the selection
effect.
...
\subsection{Challenge of the Brinson Model}
...
sector or country. One challenge of the model is to expand the
...
the number of sub-categories increases. 

Suppose an equity portfolio manager wants to find out the
contributions of any given two categories (for instance, country and
sector) to her portfolio based on the Brinson model. With respect to
each of the two categories, there is an allocation effect and a
selection effect: 
...
the difference between the actual country allocation and the passive
country allocation while assuming the same benchmark return within
each sub-category of the country factor, that is,

\begin{center}

$R_{country \, allocation} = \sum\limits_{j = 1}^N WC^P_jRC^B_j -
  \sum\limits_{j = 1}^N WC^B_jRC^B_j$,
...
\begin{itemize}
  \item $WC^P_j$ and $WC^B_j$ refer to the weight of each country $j$
    ($N$ countries in total) in the portfolio and that in the
    benchmark, respectively.
  \item $RC^B_j$ refers to the benchmark return of any country $j$.
\end{itemize}

Similarly, a sector allocation effect is the difference in return
between a portfolio sector allocation with benchmark returns and the
benchmark return according to sectors:

\begin{center}

$R_{sector \, allocation} = \sum\limits_{j = 1}^N WS^P_jRS^B_j -
  \sum\limits_{j = 1}^N WS^B_jRS^B_j$,

\end{center}

$WS^P_j$ refers to the weight of the sector $j$ in the portfolio;
$WS^B_j$, the weight of the sector $j$ in the benchmark; $RS^B_j$, the
benchmark return of any given sector $j$ of all $N$ sectors.

In the same vein, the return as a result of the selection effect
$R_{selection \, effect}$ is the sum of the country selection effect
and the sector selection effect:

\begin{center}
\begin{eqnarray[*]}
$R_{selection} & = & R_{country \, selection} + R_{sector \,
    selection} \\ 
  & = & \sum\limits_{j = 1}^N WC^B_jRC^P_j -
  \sum\limits_{j = 1}^N WC^B_jRC^B_j \\ 
  & + & \sum\limits_{j = 1}^N
  WS^B_jRS^P_j - \sum\limits_{j = 1}^N WS^B_jRS^B_j$,
\end{eqnarray[*]}
\end{center}

assuming there are $N$ sub-categories for sectors and countries,
respectively.

The interaction effect $R_{interaction}$ includes the interaction
between country allocation and sector allocation, between country
allocation and sector selection, between country selection and sector
allocation, as well as between country selection and sector
selection. 

Therefore, in the case of $Q$ categories, the Brinson model becomes
very complex:
...
effect of any given category $j$, $j$ $\in$ $Q$.
...
grows exponentially. In the case of $Q$ categories, there are $3^Q -
2Q - 1$ terms for the interaction effect. Each of the $Q$ categories
has three different characteristics -- its allocation effect,
selection effect and 1. Imagine $Q$ categories break the portfolio
into $3^Q$ cells. If a cell consists of the allocation effect of a
category $i$, 1 and 1, it represents the allocation effect of category
$i$. Should the cell include the allocation effect of a category $j$,
the selection effect of another category $k$, and 1, the cell
represents the interaction between allocation for category $j$ and
selection for category $k$. Therefore, the number of interaction terms
for $Q$ categories is $3^Q - 2Q - 1$.

Alternatively, the interaction effect can be written as,

\begin{center}

  \begin{eqnarray[*]}
  $R_{interaction} & = & \sum \limits_{j = 1}^Q \sum \limits_{k = 1}^Q R_{allocation_j}R_{allocation_k} \\
    & + & \sum \limits_{j = 1}^Q \sum \limits_{k = 1}^Q R_{allocation_j}R_{selection_k} \\
    & + & \sum \limits_{i = 1}^Q \sum \limits_{j = 1}^Q \sum \limits_{k = 1}^Q R_{allocation_i}R_{allocation_j}R_{selection_k} + \dots$.
  \end{eqnarray[*]}
\end{center}

For instance, when there are 3 categories, $3^3 - 2*3 - 1 = 20$
interaction terms have to be estimated. When there are 4 categories,
$3^4 - 2*4 - 1 = 72$ interaction terms have to be estimated. The
number of interaction terms increases exponentially as the number of
categories in the Brinson model increases. This poses a significant
computational challenge when a portfolio manager performs a
multi-variate Brinson analysis.
...
2010. This plot suggests that Utilities performed the best with a gain
of more than 5\% and Consumer Discretionary, the worst performing
sector, lost more than 10\%.
...
\item $\mathbf{X_{n,k}}$ is an $n$ by $k$ matrix. Each row represents
...
of all her three holdings. The matrix $\mathbf{X_{3,3}}$ records the
score for each of the three factors (value, growth, and momentum) in
each row. $\mathbf{f_3}$ contains the estimated coefficients of a
regression $\mathbf{r_3}$ on $\mathbf{X_{3, 3}}$.
...
attribute $i$ in the portfolio, $\mathbf{w_{active}}$ is a column
vector of length $n$ containing the active weight of every security in
the portfolio, and $\mathbf{x_{n}}$ is a column vector of length $n$
...
$\mathbf{w_{active}}'$ (containing active weight of each of the three
holdings) and $\mathbf{x_{3}}$ (containing value scores of the three
holdings).
...
value are -0.03125 and -0.12500. The active exposure of size is 0.32
...
Another way to think about the analysis as \cite{brinson:gary} have done is
to consider it in the context of a regression model. Conducting a
Brinson attribution is similar to running a linear regression without
the intercept term. Estimated coefficients will then be the mean
return of each category in the universe, a.k.a. the factor return of
each category. The mean return of each category also appears in the
Brinson analysis. The equivalent to the allocation effect for the
universe in the Brinson model is the sum of the product of the
estimated coefficient and the active weight of each category. Active
weight of a category is defined as the difference between portfolio
weight and benchmark weight for that category.
...
weight of each category as specified by the manager; $\mathbf{W^B}$, a
column vector indicating the benchmark weight of each category as
specified by the manager, and $\mathbf{f}$ is the column vector which
has benchmark return of all the categories. Assuming that in this
case, the universe is the same as the benchmark, $\mathbf{R^B}$ can be
estimated by regressing returns on the attributes specified by the
manager:

\begin{center}
  $\mathbf{r_n} = \mathbf{X_{n,k}f} + \mathbf{U}$
\end{center}
...
\item $\mathbf{f}$ is the estimated coefficients on the regression
  without the intercept term. To satisfy the linear regression, the
  estimated coefficient of each attribute is the mean return for each
  of the attribute.
\item $\mathbf{U}$ is the column vector with all the residual terms. 
\end{itemize}
  
Therefore, the allocation effect in the Brinson model is a special
...
\begin{center}
  \begin{eqnarray[*]}
    $R_{selection}$ & = & $\sum\limits_{j = 1}^N W^B_jR^P_j - \sum\limits_{j =
      1}^N W^B_jR^B_j$ \\
    & = & $\mathbf{W^B}\prime (\mathbf{f^P} - \mathbf{f^B})$,	
  \end{eqnarray[*]}
\end{center}
...
$\mathbf{f^B}$ are the column vectors indicating the mean returns of
the portfolio and the benchmark, respectively. As mentioned above,
$\mathbf{f^P}$ and $\mathbf{f^B}$ can be estimated by running a linear
regression without the intercept term with respect to stocks in the
portfolio and benchmark separately. Hence, the selection effect in the
Brinson model can be calculated by using linear regression.
...
%% data(jan)
%% test <- jan
%% test$benchmark <- 1/3000
%% set.seed(8)
%% test$portfolio <- 0
%% test$portfolio[sample(3000, 300)] <- 1/300
%% head(test)
%% test <- test[ ,c(2, 3, 4, 5, 14:15)]
%% getwd()
...
data(test)
## allocation effect
br.single <- brinson(x = test, date.var = "date", 
                     cat.var = "sector",
                     bench.weight = "benchmark", 
                     portfolio.weight = "portfolio", 
                     ret.var = "return")
...
Below shows that running a regression without the intercept term and
apply the procedure as mentioned above yeilds the same allocation
effect, -3.4 bps.
...
Similarly, applying the regression method produces the same selection
effect as done by the Brinson model -- -4.25 bps.

<<>>=
## selection effect
lm.p <- lm(return ~ sector - 1, 
           data = test[test$portfolio != 0, ])
lm.p$coefficients 
exposure(br.single, var = "sector")[ ,2] %*% 
  (lm.p$coefficients - lm.1$coefficients)
@ 
...
returns for equity portfolios. It is possible to use the \pkg{pa}
package to work on the output generated by the \pkg{portfolio}
package. Further, the flexibility of \R{} itself allows users to
extend and modify these packages to suit their own needs and/or
execute their preferred attribution methodology. Before reaching that
level of complexity, however, \pkg{pa} provides a good starting point
