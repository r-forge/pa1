...
Regression-based analysis [\cite{jpmreg}]. \cite{brinson:gary}
takes an ANOVA-type approach and decomposes active return of any given
portfolio into asset allocation, stock selection, and interaction
effects. The Regression-based analysis utilizes the estimated
...
doing better than the sector as a whole. Call this the \emph{selection
  effect}. The rest of her outperformance can then be attributed to an
interaction between allocation and selection: \emph{interaction
  effect}. The Brinson Model provides mathematical definitions for
these terms and methods of calculating them.

There are two ways to calculate the return of a portfolio, essentially
an aggregation of return on either an individual security level or a
\emph{category} level. Category is a user-defined term based on
certain characteristic of a portfolio, such as sector and
country. Even a continuous variable can be treated as a category by
dividing it into discrete buckets, for example, quintiles.

We define some symbols as follows:

\begin{itemize}
\item $w_i$ is the weight of a security $i$.
\item $r_i$ is the return of a security $i$.
\item $w^b_i$ is the weight of a security $i$ in the benchmark.
\item $w^p_i$ is the weight of a security $i$ in the portfolio.
\item $C^b_j$ is the weight of a category $j$ in the benchmark. $C^b_j
...
\item $R^p_j$ is the return of a category $j$ in the portfolio. $R^b_j
  = \sum w^p_ir_i$, $i$ is a member of the category $j$.
\end{itemize}

We can thus calculate the various return metrics of the portfolio
and the benchmark.
...
%   \item $w^p_j$ is the weight of each sector $j$ in the equity
%     portfolio. The weight of each sector can be calculated by
%     $\sum_{i}w_{p_i}$ where $w_{p_i}$ refers to the weight of each
%     equity in the chosen sector in the portfolio.
%   \item $w^b_j$ is the weight of each sector $j$ in the
%     benchmark. Similarly, the weight of each sector can be calculated
%     by $\sum_{i}w_{b_i}$ where $w_{b_i}$ refers to the weight of each
%     equity in the chosen sector in the benchmark.
%   \item $r^p_j$ is the return of each sector $j$ in the equity
%     portfolio. The return of each sector in the portfolio can be
%     calculated by $\sum w_{p_i}r_{p_i}$ where $w_{p_i}$ and $r_{p_i}$
%     refer to the weight and return of each equity in the chosen sector
%     in the portfolio, respectively.
%   \item $r^b_j$ is the return of each sector $j$ in the benchmark. The
%     return of each sector in the benchmark can be calculated by $\sum
%     w_{b_i}r_{b_i}$ where $w_{b_i}$ and $r_{b_i}$ refer to the weight
%     and return of each equity in the chosen sector in the benchmark,
%     respectively.
%   \item $r_p$ is the return of the equity portfolio.
%   \item $r_b$ is the return of the benchmark.
%   \item Return of the original equity portfolio $r_p$ = $\sum_{j}
%     w^p_jr^p_j$.
%   \item Benchmark return $r_b$ = $\sum_{j} w^b_jr^b_j$.
%   \item Active return of the equity portfolio $r_{active}$ = $r_p -
%     r_b$.
% \end{itemize}


%% talk about sector allocation, stock selection
...
return of the portfolio by any category. One challenge faced by the
...
% To expand beyond two
% variables requires much more sophistication and is almost impossible
% to achieve. Further, the Brinson Model also has difficulty in
% analyzing the contribution of a continuous variable though
% empirically, portfolio managers could break a continous variable into
% various quantiles.
...
%% Another point to make is that a problem with the brinson model is to
%% difficult to expand to more than one factor. ``no longer works'' is
%% wrong,. reference the 2nd article, expand it to more than one factor.
...
%% mean return to a sector, i also know the active weight of each
%% sector. 10\% financials in portfolio, 8\% in benchmark, 2\% active
%% weight.

%% return of each sector is known.

%% 10\% return from financial sector, 2\% active weight.

%% what's the equivalent of allocation effect

%% running a regression, coefficients times active weights
...
%% However, one would not be able to derive the selection effect in the
%% Brinson analysis from the aforementioned procedure. In order to
%% calculate the selection effect, one has to take into account
%% stock-specific characteristics such as market capitalization and
%% various style factors.

%% in a regression approach, not getting selection effect. in order to
%% see selection effect, have to start looking at stock specific
%% things. calculate a selection effect within a single factor at a
%% time. regress returns for just financial stocks. take the residual of
%% the regression, multiply by active weights, will get selection effect.
...
%% return = country, sector, size
%% other variables are also possible.
%% y = x1 + x2 + (sector has dummy variables).
%% run a regression, coefficients of size are estimated.
%% active exposure calculated
%% attribution to size = coeff * exposure.
%% if active weight of size is 0. no effect.
%% The higher exposure to size, the more influence it has.
%% regression run is predict returns for individual companies -- sector,
%% country and size. difference between the predicted ret and the real
%% value is the residual.

%% Regression allows continuous var.

%% exposure for caps, coefficients times exposures

%% regress with terms on sectors with no intersection.
...
[\cite{kane:david}], provides facilities to calculate exposure and
return of an equity portfolio by a given factor. One can then run the
Brinson analysis based on the output generated by the \pkg{portfolio}
...
for executing the analyses as mentioned above.
