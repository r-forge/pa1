...
in arithmetic attribution is that the arithmetic summation of
single-period active returns is not equal to the actual active return
over time,
...
where $b_t^{opt}$ is the optimized linking coefficient in period $t$.
